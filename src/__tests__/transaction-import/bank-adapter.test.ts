...
vi.mock('../../lib/transaction-import/csv-parser', () => ({
  parseCSV: vi.fn().mockResolvedValue({
    transactions: [{ date: '2026-01-01', merchant: 'Test', amount: 100, type: 'debit' as const, balance: 900, rawData: {} }],
    errors: [],
    totalRows: 1,
    successfulRows: 1,
  }),
...
describe('Bank Adapter', () => {
  ...
  describe('parseBankStatement', () => {
    ...
    it('for CSV with valid bank code returns ParseResult', async () => {
      const content = 'Date,Details,Debit,Credit,Balance\n01/01/2026,Test,100,0,900';
    ...
    it('throws for invalid bank code on CSV', async () => {
      await expect(parseBankStatement('a,b,c', 'INVALID', 'csv')).rejects.toThrow(/Unsupported bank/);
    });
  });
});
