...
vi.mock('@/lib/supabase/server', () => ({
  createServerClient: vi.fn(() =>
    Promise.resolve({
      ...
      from: vi.fn(() => ({ insert: vi.fn(), select: vi.fn(), eq: vi.fn(), update: vi.fn() })),
    })
  ),
}));
...
describe('POST /api/transactions/upload-v2', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    form.set('file', new Blob(['x'.repeat(100)], { type: 'application/pdf' }), 'test.pdf');
    form.set('bankCode', 'GTB');

    const req = new NextRequest('http://localhost:3000/api/transactions/upload-v2', {
      method: 'POST',
      body: form,
    });
