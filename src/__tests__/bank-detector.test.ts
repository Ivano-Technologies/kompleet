...
import { detectBankFromCSV, detectBank } from '@/lib/transaction-import/bank-detector';
...
describe('Bank Detector', () => {
  describe('detectBankFromCSV', () => {
    it('should detect Moniepoint from CSV file', async () => {
      const filePath = join(process.cwd(), 'tests/fixtures/banks/moniepoint_sample1.csv');
      const fileBuffer = readFileSync(filePath);
      
      const result = await detectBankFromCSV(fileBuffer, 'moniepoint_sample1.csv');
    ...
    it('should detect GTBank from CSV file', async () => {
      const filePath = join(process.cwd(), 'tests/fixtures/banks/gtbank_sample1.csv');
      const fileBuffer = readFileSync(filePath);
    ...
    it('should detect bank from filename pattern', async () => {
      ...
      const result = await detectBankFromCSV(fileBuffer, 'my_moniepoint_statement.csv');
      
      expect(result.bankCode).toBe('MON');
      expect(result.matchedFeatures.some(f => f.includes('Filename'))).toBe(true);
    });
  });
  ...
  describe('Header Matching', () => {
    it('should match headers with high confidence', async () => {
      // Moniepoint headers
      const csvContent = 'Date,Narration,Debit,Credit,Balance,Reference\n15/01/2026,Test,,1000,5000,REF001';
