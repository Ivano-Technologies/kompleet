...
import { parseBankStatement, detectFileType, isValidBankCode } from '@/lib/transaction-import/bank-adapter';
...
async function handlePOST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const normalizedTransactions = normalizeTransactions(parseResult.transactions, bankCode);
      ...
      const transactionsToImport = normalizedTransactions.filter(
        t => !highConfidenceDuplicates.some(d => 
      ...
      await supabase
        ...
        .update({
          ...
          total_amount: transactionsToImport.reduce((sum, t) => sum + t.amount, 0),
