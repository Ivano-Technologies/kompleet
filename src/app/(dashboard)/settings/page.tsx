...
export default function SettingsPage() {
  ...
  const Toggle = ({ on, onToggle }: { on: boolean; onToggle: () => void }) => (
    <button
      onClick={onToggle}
      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${on ? "bg-primary-500" : "bg-light-border dark:bg-dark-border dark:bg-light-text-tertiary dark:bg-dark-text-tertiary"}`}
    >
      <span
        className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${on ? "translate-x-[18px]" : "translate-x-[3px]"}`}
      />
    </button>
  );
      ...
      <div className="p-5 rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            ...
            <button className="ml-2 text-xs text-light-text-tertiary dark:text-dark-text-tertiary hover:text-light-text-primary dark:hover:text-dark-text-primary">
              Remove
            </button>
            <p className="text-[10px] text-light-text-tertiary dark:text-dark-text-tertiary mt-1">
              JPG, PNG. Max 2MB. 300×300px min.
            </p>
          </div>
        </div>
          ...
          {twoFactorEnabled && (
            <div className="mt-3 p-2.5 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              <p className="text-xs text-green-700 dark:text-green-400 flex items-center gap-1.5">
                <CheckCircle className="w-3 h-3" /> 2FA is enabled
              </p>
            </div>
          )}
        </div>
      ...
      <div className="p-5 rounded-xl border-2 border-red-200 dark:border-red-800/30 bg-light-surface dark:bg-dark-surface">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-1.5 rounded-lg font-medium transition-colors flex items-center gap-1 disabled:opacity-50"
          >
            {deleteLoading ? (
              <>
