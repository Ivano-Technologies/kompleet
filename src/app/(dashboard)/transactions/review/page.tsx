...
export default function TransactionReviewPage() {
  ...
  const generateSuggestions = (transaction: Transaction) => {
    ...
    categories.forEach((category) => {
      ...
      if (transaction.transaction_type === 'credit' && category.category_type === 'income') {
        confidence += 20;
        reasons.push('Credit transaction suggests income');
      } else if (transaction.transaction_type === 'debit' && category.category_type === 'expense') {
        confidence += 20;
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-12 text-center">
          ...
          <p className="text-green-700 mb-6">
            All your transactions are categorized or have high confidence scores.
          </p>
          <Link
            href="/transactions"
        ...
        {currentTransaction.category && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800">
              Currently categorized as <strong>{currentTransaction.category.name}</strong>
              {currentTransaction.confidence_score !== undefined && (
                <span> ({currentTransaction.confidence_score}% confidence)</span>
              )}
            </p>
          </div>
        )}
