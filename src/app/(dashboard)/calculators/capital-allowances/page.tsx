...
export default function CapitalAllowancesCalculator() {
  ...
  const calculateCapitalAllowance = () => {
    ...
    try {
      ...
      // Calculate first year allowance (pro-rated)
      ...
      // Generate depreciation schedule (10 years)
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Capital Allowances Calculator</h1>
      ...
      <Card className="p-6 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">Enter your asset details</p>
      ...
      {result && (
        <Card className="p-6 mb-6 bg-green-50 border-green-200">
          <h2 className="text-xl font-semibold mb-4">Capital Allowance Result</h2>

          <div className="space-y-3 mb-6">
            <div className="flex justify-between">
              <span className="font-medium">Asset Cost:</span>
              <span>{formatCurrency(result.assetCost)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Allowance Rate:</span>
              <span>{result.allowanceRate}%</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Annual Allowance:</span>
              <span className="font-semibold">{formatCurrency(result.annualAllowance)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">First Year Allowance (Pro-rated):</span>
              <span className="font-semibold">{formatCurrency(result.firstYearAllowance)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Years to Full Depreciation:</span>
              <span>{result.yearsToFullDepreciation} years</span>
            </div>
          </div>

          <div className="border-t pt-4">
            <h3 className="font-semibold mb-3">Depreciation Schedule</h3>
                ...
                <tbody>
                  {result.schedule.map((row) => (
                    <tr key={row.year} className="border-b">
                      ...
                      <td className="text-right p-2">{formatCurrency(row.allowance)}</td>
