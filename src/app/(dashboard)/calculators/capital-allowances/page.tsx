  ...
  const calculateCapitalAllowance = () => {
    ...
    try {
      ...
      const monthsOwned = Math.ceil(
        (yearEnd.getTime() - acquisitionDateObj.getTime()) /
          (1000 * 60 * 60 * 24 * 30),
      );
    ...
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">
            Calculate annual depreciation allowances for business assets under
          ...
          <div className="space-y-3 mb-6 text-light-text-secondary dark:text-dark-text-secondary">
            <div className="flex justify-between">
              ...
              <span className="text-light-text-primary dark:text-dark-text-primary">
                {formatCurrency(result.assetCost)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Allowance Rate:</span>
              <span className="text-light-text-primary dark:text-dark-text-primary">
                {result.allowanceRate}%
              </span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Annual Allowance:</span>
              <span className="font-semibold text-primary-600 dark:text-primary-400">
                {formatCurrency(result.annualAllowance)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">
                First Year Allowance (Pro-rated):
              </span>
              <span className="font-semibold text-primary-600 dark:text-primary-400">
                {formatCurrency(result.firstYearAllowance)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Years to Full Depreciation:</span>
              <span className="text-light-text-primary dark:text-dark-text-primary">
                {result.yearsToFullDepreciation} years
              </span>
            </div>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-light-border dark:border-dark-border">
                    <th className="text-left p-2 font-medium text-light-text-secondary dark:text-dark-text-secondary">
                      Year
                    </th>
                    <th className="text-right p-2 font-medium text-light-text-secondary dark:text-dark-text-secondary">
                      Opening Value
                    </th>
                    <th className="text-right p-2 font-medium text-light-text-secondary dark:text-dark-text-secondary">
                      Allowance
                ...
                <tbody className="text-light-text-secondary dark:text-dark-text-secondary">
                  {result.schedule.map((row) => (
                    ...
                    >
                      ...
                      <td className="text-right p-2 text-primary-600 dark:text-primary-400">
      ...
      <Card className="p-5 rounded-xl border border-blue-500/20 bg-blue-500/5 dark:bg-blue-500/10">
        ...
        <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mb-4">
          ...
          capital allowances. Allowance rates are fetched dynamically from the
          KOMPLEET Tax Rules Engine.
        </p>
