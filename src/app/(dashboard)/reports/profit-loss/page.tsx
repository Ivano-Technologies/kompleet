...
export default function ProfitLossPage() {
  ...
  const profitMargin = statement && statement.revenue.total > 0
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold text-light-text-primary dark:text-dark-text-primary">Profit & Loss Statement</h1>
        <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-1">Income Statement following Nigerian accounting standards</p>
      </div>

      {/* Date Selection */}
      <div className="p-5 rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
        <h2 className="text-sm font-semibold text-light-text-primary dark:text-dark-text-primary mb-3">Select Period</h2>
      ...
      {statement && (
        <div className="p-6 rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
          <div className="text-center mb-6 pb-4 border-b-2 border-light-text-primary dark:border-dark-text-primary">
            <h2 className="text-lg font-bold text-light-text-primary dark:text-dark-text-primary">PROFIT & LOSS STATEMENT</h2>
            <p className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary mt-1">
              {fmtDate(statement.period.start)} to {fmtDate(statement.period.end)}
            </p>
          </div>

          {/* Revenue */}
          <div className="mb-5">
            <h3 className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-2 pb-1.5 border-b border-light-border dark:border-dark-border">REVENUE</h3>
            {statement.revenue.items.length > 0 ? (
              <table className="w-full text-sm">
                <tbody>
                  {statement.revenue.items.map((item, idx) => (
                    <tr key={idx} className="border-b border-light-border/50 dark:border-dark-border/50">
                      <td className="py-2 text-light-text-secondary dark:text-dark-text-secondary">
                        {item.category} <span className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary">({item.count})</span>
                      </td>
                      <td className="py-2 text-right font-medium text-light-text-primary dark:text-dark-text-primary">{fmt(item.amount)}</td>
                    </tr>
                  ))}
                  <tr className="border-t-2 border-light-text-primary dark:border-dark-text-primary font-bold">
                    <td className="py-2.5 text-light-text-primary dark:text-dark-text-primary">TOTAL REVENUE</td>
                    <td className="py-2.5 text-right text-light-text-primary dark:text-dark-text-primary">{fmt(statement.revenue.total)}</td>
                  </tr>
                </tbody>
              </table>
            ) : (
              <p className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary italic py-2">No revenue transactions</p>
            )}
          </div>

          {/* Expenses */}
          <div className="mb-5">
            <h3 className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-2 pb-1.5 border-b border-light-border dark:border-dark-border">EXPENSES</h3>
            {statement.expenses.items.length > 0 ? (
              <table className="w-full text-sm">
                <tbody>
                    ...
                    <tr key={idx} className="border-b border-light-border/50 dark:border-dark-border/50">
                      <td className="py-2 text-light-text-secondary dark:text-dark-text-secondary">
                        ...
                        {item.taxTreatment === 'non_deductible' && (
                          <span className="ml-1.5 text-xs bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400 px-1.5 py-0.5 rounded">Non-deductible</span>
                        )}
                      </td>
                      <td className="py-2 text-right font-medium text-light-text-primary dark:text-dark-text-primary">{fmt(item.amount)}</td>
                    </tr>
                  ...
                  <tr className="border-t-2 border-light-text-primary dark:border-dark-text-primary font-bold">
                    <td className="py-2.5 text-light-text-primary dark:text-dark-text-primary">TOTAL EXPENSES</td>
                    <td className="py-2.5 text-right text-light-text-primary dark:text-dark-text-primary">{fmt(statement.expenses.total)}</td>
                  </tr>
                </tbody>
              </table>
            ) : (
              <p className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary italic py-2">No expense transactions</p>
            )}
          </div>
            ...
            <table className="w-full">
              <tbody>
                <tr className="text-base font-bold">
                  <td className={`py-2 ${statement.netIncome >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
                    NET {statement.netIncome >= 0 ? 'PROFIT' : 'LOSS'}
                  </td>
                  <td className={`py-2 text-right ${statement.netIncome >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
                ...
                <tr className="font-semibold border-t border-light-border dark:border-dark-border text-sm">
                  <td className="py-2 text-light-text-primary dark:text-dark-text-primary">
                    TAXABLE INCOME <span className="text-xs font-normal text-light-text-tertiary dark:text-dark-text-tertiary">(Excl. non-deductible)</span>
                  </td>
                  <td className="py-2 text-right text-light-text-primary dark:text-dark-text-primary">{fmt(statement.taxableIncome)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 pt-4 border-t border-light-border dark:border-dark-border">
            {[
              { label: 'Profit Margin', value: `${profitMargin}%`, color: 'text-blue-600 dark:text-blue-400' },
              { label: 'Total Revenue', value: fmt(statement.revenue.total), color: 'text-green-600 dark:text-green-400' },
              { label: 'Total Expenses', value: fmt(statement.expenses.total), color: 'text-red-600 dark:text-red-400' },
            ].map((m) => (
      ...
      {!statement && !loading && (
        <div className="py-12 text-center rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
          ...
          <h3 className="text-sm font-semibold text-light-text-primary dark:text-dark-text-primary mb-1">Generate Your Statement</h3>
          <p className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary">Select a date range above to view your income and expenses</p>
        </div>
      )}
    </div>
  );
