...
export default function BalanceSheetPage() {
  ...
  const currentRatio =
    statement && statement.liabilities.current.total > 0
      ? (
          statement.assets.current.total / statement.liabilities.current.total
        ).toFixed(2)
      : "N/A";
  const debtToEquity =
    statement && statement.equity.total > 0
      ...
      <table className="w-full text-sm">
        <tbody>
          {items.map((item, i) => (
            ...
            >
              <td className="py-1.5 text-light-text-secondary dark:text-dark-text-secondary">
                {item.category}
              </td>
              <td className="py-1.5 text-right font-medium text-light-text-primary dark:text-dark-text-primary">
                {fmt(item.amount)}
              </td>
            </tr>
          ))}
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-1">
          Statement of Financial Position — Nigerian accounting standards
        </p>
      </div>
        ...
        <div className="p-6 rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
          <div className="text-center mb-6 pb-4 border-b-2 border-light-text-primary dark:border-dark-text-primary">
            ...
            <p className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary mt-1">
              As of {fmtDate(statement.period.asOf)}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Assets */}
            <div>
              <h3 className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-3 pb-1.5 border-b-2 border-light-text-primary dark:border-dark-text-primary">
                ASSETS
              </h3>
              <div className="mb-4">
                <h4 className="text-xs font-semibold text-light-text-secondary dark:text-dark-text-secondary mb-2 uppercase tracking-wide">
                  Current Assets
                </h4>
                <SectionTable
                  items={statement.assets.current.items}
                  totalLabel="Total Current"
                  total={statement.assets.current.total}
                />
              </div>
              <div className="mb-4">
                <h4 className="text-xs font-semibold text-light-text-secondary dark:text-dark-text-secondary mb-2 uppercase tracking-wide">
                ...
                <SectionTable
                  items={statement.assets.nonCurrent.items}
                  totalLabel="Total Non-Current"
                  total={statement.assets.nonCurrent.total}
                />
              </div>
              <div className="border-t-4 border-light-text-primary dark:border-dark-text-primary pt-2">
                <div className="flex justify-between text-base font-bold text-light-text-primary dark:text-dark-text-primary py-2">
                  ...
                  <span>{fmt(statement.assets.total)}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <SectionTable
                  items={statement.liabilities.current.items}
                  totalLabel="Total Current"
                  total={statement.liabilities.current.total}
                />
              </div>
              <div className="mb-4">
                <h4 className="text-xs font-semibold text-light-text-secondary dark:text-dark-text-secondary mb-2 uppercase tracking-wide">
                ...
                <SectionTable
                  items={statement.liabilities.nonCurrent.items}
                  totalLabel="Total Non-Current"
                  total={statement.liabilities.nonCurrent.total}
                />
              </div>
              <div className="mb-4">
                <h4 className="text-xs font-semibold text-light-text-secondary dark:text-dark-text-secondary mb-2 uppercase tracking-wide">
                ...
                <SectionTable
                  items={statement.equity.items}
                  totalLabel="Total Equity"
                  total={statement.equity.total}
                />
              </div>
              <div className="border-t-4 border-light-text-primary dark:border-dark-text-primary pt-2">
                <div className="flex justify-between text-base font-bold text-light-text-primary dark:text-dark-text-primary py-2">
                  <span>TOTAL L & E</span>
                  <span>{fmt(statement.totalLiabilitiesAndEquity)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 pt-4 border-t border-light-border dark:border-dark-border">
            {[
              ...
              {
                label: "Total Assets",
                value: fmt(statement.assets.total),
      ...
      {!statement && !loading && (
        <div className="py-12 text-center rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
          ...
          <p className="text-xs text-light-text-tertiary dark:text-dark-text-tertiary">
            Select a date above to view assets, liabilities, and equity
          </p>
        </div>
      )}
    </div>
