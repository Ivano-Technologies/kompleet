...
export default function ReportsClient({ stats }: ReportsClientProps) {
  ...
  const dateRange = {
    start: today.toLocaleDateString("en-NG", {
      month: "short",
      day: "numeric",
      year: "numeric",
    }),
    end: futureDate.toLocaleDateString("en-NG", {
      month: "short",
      day: "numeric",
      year: "numeric",
    }),
    ...
    },
    {
      ...
      value:
        stats.totalRevenue > 0
          ? `${(((stats.netIncome * 0.3) / stats.totalRevenue) * 100).toFixed(2)}%`
          : "0%",
      change: "+4%",
      up: true,
      icon: LineChart,
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${report.badgeColor}`}
                >
                  {report.badge}
                </span>
              </div>
              ...
              <ul className="space-y-1.5 mb-5">
                {report.items.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-2 text-xs text-light-text-tertiary dark:text-dark-text-tertiary"
                  >
