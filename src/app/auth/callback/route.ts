...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL(
        `/login?error=${mappedError}&details=${encodeURIComponent(errorDescription || "")}`,
        requestUrl.origin,
      ),
    );
  }
  ...
  try {
    ...
    if (isPasswordRecovery) {
      ...
      if (recoveryError) {
        ...
        if (recoveryError.message.includes("expired")) {
          return NextResponse.redirect(
            new URL(
              "/login?error=expired_link&message=Password reset link has expired. Please request a new one.",
              requestUrl.origin,
            ),
          );
        }
        ...
        return NextResponse.redirect(
          new URL(
            "/login?error=invalid_recovery_link&message=Invalid password reset link. Please request a new one.",
            requestUrl.origin,
          ),
        );
      }
    ...
    if (exchangeError) {
      ...
      if (exchangeError.message.includes("expired")) {
        return NextResponse.redirect(
          new URL(
            "/login?error=expired_link&message=This link has expired. Please request a new one.",
            requestUrl.origin,
          ),
        );
      }
      ...
      if (exchangeError.message.includes("already used")) {
        return NextResponse.redirect(
          new URL(
            "/login?error=link_used&message=This link has already been used. Please request a new one.",
            requestUrl.origin,
          ),
        );
      }
      ...
      return NextResponse.redirect(
        new URL(
          "/login?error=auth_failed&message=Authentication failed. Please try again.",
          requestUrl.origin,
        ),
      );
    }
    ...
    if (!data.session) {
      ...
      return NextResponse.redirect(
        new URL(
          "/login?error=no_session&message=Failed to create session. Please try again.",
          requestUrl.origin,
        ),
      );
    }
    ...
    const authType = redirect.includes("reset-password")
      ? "password_reset"
      : "login";
    console.log(
      `Auth callback success - Type: ${authType}, User: ${data.user?.email}, Redirect: ${redirect}`,
    );
    ...
    const isAllowedRedirect = allowedRedirects.some((allowed) =>
      redirect.startsWith(allowed),
    );

    if (!isAllowedRedirect) {
      console.warn(
        `Invalid redirect URL attempted: ${redirect}, defaulting to /dashboard`,
      );
