...
export async function prefillPITForm(
  ...
  try {
    ...
    const withholdingTax = grossIncome * 0.1;

    // Calculate PIT
    const { consolidatedRelief, taxableIncome, taxPayable } =
      calculatePIT(grossIncome);
...
export async function prefillCITForm(
  ...
  try {
    ...
    const costOfSales = turnover * 0.6;
    ...
    const capitalAllowances = profitBeforeTax * 0.1;
