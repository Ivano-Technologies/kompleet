...
const GMAIL_REDIRECT_URI = process.env.NEXT_PUBLIC_APP_URL + '/api/email/callback/gmail';
...
export async function fetchEmails(
  ...
  const query = options.query || 'subject:(receipt OR invoice OR payment) OR from:(*@shoprite.* OR *@jumia.* OR *@konga.*)';
...
export function parseEmailForTransaction(message: any): {
  ...
  try {
    ...
    // Extract date
    const dateHeader = payload.headers.find((h: any) => h.name === 'Date')?.value;
