...
const DEFAULT_THRESHOLDS: QualityThresholds = {
  ...
  autoCategorizedRate: 0.7,
  manualOverrideRate: 0.15,
  duplicateDetectionRate: 0.9,
...
class DataQualityMonitor {
  ...
  recordParseResult(total: number, successful: number, errors: number): void {
    ...
    this.metrics.totalTransactionsProcessed =
      (this.metrics.totalTransactionsProcessed || 0) + successful;
    this.metrics.totalErrors = (this.metrics.totalErrors || 0) + errors;

    console.log(
      `Parse result recorded: ${successful}/${total} successful (${(this.metrics.parseSuccessRate * 100).toFixed(1)}%)`,
    );
  }
  ...
  recordCategorization(auto: number, manual: number, total: number): void {
    if (total > 0) {
      ...
      console.log(
        `Categorization recorded: ${auto} auto, ${manual} manual out of ${total}`,
      );
    }
  }
  ...
  recordDuplicateDetection(
    ...
    if (total > 0) {
      ...
      console.log(
        `Duplicate detection recorded: ${detected}/${total} detected, ${falsePositives} false positives`,
      );
    }
  }
  ...
  meetsQualityStandards(): { passes: boolean; failures: string[] } {
    ...
    if (
      this.metrics.parseSuccessRate !== undefined &&
      this.metrics.parseSuccessRate < this.thresholds.parseSuccessRate
    ) {
      failures.push(
        `Parse success rate ${(this.metrics.parseSuccessRate * 100).toFixed(1)}% below target ${(this.thresholds.parseSuccessRate * 100).toFixed(1)}%`,
      );
    }

    if (
      this.metrics.autoCategorizedRate !== undefined &&
      this.metrics.autoCategorizedRate < this.thresholds.autoCategorizedRate
    ) {
      failures.push(
        `Auto-categorization rate ${(this.metrics.autoCategorizedRate * 100).toFixed(1)}% below target ${(this.thresholds.autoCategorizedRate * 100).toFixed(1)}%`,
      );
    }

    if (
      this.metrics.manualOverrideRate !== undefined &&
      this.metrics.manualOverrideRate > this.thresholds.manualOverrideRate
    ) {
      failures.push(
        `Manual override rate ${(this.metrics.manualOverrideRate * 100).toFixed(1)}% above target ${(this.thresholds.manualOverrideRate * 100).toFixed(1)}%`,
      );
    }

    if (
      this.metrics.duplicateDetectionRate !== undefined &&
      this.metrics.duplicateDetectionRate <
        this.thresholds.duplicateDetectionRate
    ) {
      failures.push(
        `Duplicate detection rate ${(this.metrics.duplicateDetectionRate * 100).toFixed(1)}% below target ${(this.thresholds.duplicateDetectionRate * 100).toFixed(1)}%`,
      );
    }

    if (
      this.metrics.falsePositiveRate !== undefined &&
      this.metrics.falsePositiveRate > this.thresholds.falsePositiveRate
    ) {
      failures.push(
        `False positive rate ${(this.metrics.falsePositiveRate * 100).toFixed(1)}% above target ${(this.thresholds.falsePositiveRate * 100).toFixed(1)}%`,
      );
    }
...
function formatMetric(
  ...
  const displayValue =
    format === "percentage" ? `${(value * 100).toFixed(1)}%` : value.toFixed(2);
  const targetValue =
    format === "percentage"
...
function generateRecommendations(
  ...
  if (
    metrics.parseSuccessRate !== undefined &&
    metrics.parseSuccessRate < thresholds.parseSuccessRate
  ) {
  ...
  if (
    metrics.autoCategorizedRate !== undefined &&
    metrics.autoCategorizedRate < thresholds.autoCategorizedRate
  ) {
  ...
  if (
    metrics.manualOverrideRate !== undefined &&
    metrics.manualOverrideRate > thresholds.manualOverrideRate
  ) {
    recommendations.push(
      "• Analyze manual overrides to improve auto-categorization",
    );
    recommendations.push("• Add new rules based on common override patterns");
  }

  if (
    metrics.duplicateDetectionRate !== undefined &&
    metrics.duplicateDetectionRate < thresholds.duplicateDetectionRate
  ) {
  ...
  if (
    metrics.falsePositiveRate !== undefined &&
    metrics.falsePositiveRate > thresholds.falsePositiveRate
  ) {
