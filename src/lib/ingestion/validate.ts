...
export function validateTransaction(
  ...
  if (!transaction.date) {
    ...
    if (isNaN(date.getTime())) {
      errors.push({ type: 'INVALID_DATE', message: `Invalid date: ${transaction.date}` });
    } else {
  ...
  if (!transaction.description) {
    errors.push({ type: 'MISSING_DESCRIPTION', message: 'Description is required' });
  ...
  if (transaction.amount === undefined || transaction.amount === null) {
    ...
    if (!Number.isFinite(transaction.amount)) {
      errors.push({ type: 'INVALID_AMOUNT', message: `Invalid amount: ${transaction.amount}` });
  ...
  // Validate type
  if (!transaction.type) {
    errors.push({ type: 'MISSING_TYPE', message: 'Transaction type is required' });
  } else if (rules.allowedTypes && !rules.allowedTypes.includes(transaction.type)) {
    errors.push({
      type: 'INVALID_TYPE',
  ...
  // Validate currency
  if (transaction.currency && rules.allowedCurrencies && !rules.allowedCurrencies.includes(transaction.currency)) {
    errors.push({
  ...
  if (!transaction.source_file_id) {
    errors.push({ type: 'MISSING_SOURCE_FILE_ID', message: 'Source file ID is required' });
  }
