...
describe('validateEnv', () => {
  ...
  it('should pass validation with all required vars', () => {
    ...
    if (result.valid) {
      expect(result.env.NEXT_PUBLIC_SUPABASE_URL).toBe('https://example.supabase.co');
    }
  });
  ...
  it('should fail validation with missing required vars', () => {
    ...
    if (!result.valid) {
      ...
      expect(result.errors.some(err => err.includes('NEXT_PUBLIC_SUPABASE_ANON_KEY'))).toBe(true);
    }
  });
  ...
  it('should detect potentially exposed secrets', () => {
    ...
    expect(result.valid).toBe(false);
    if (!result.valid) {
      expect(result.errors.some(err => err.includes('NEXT_PUBLIC_SECRET_KEY'))).toBe(true);
    }
  });
});
