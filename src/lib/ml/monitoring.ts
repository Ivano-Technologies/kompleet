...
export async function getModelMetrics(
  ...
  const avgLatency = inferences.reduce((sum, i) => sum + i.latency_ms, 0) / inferenceCount;
  const avgConfidence = inferences.reduce((sum, i) => sum + i.confidence, 0) / inferenceCount;
  
  // Calculate accuracy from corrections
  const correctionCount = corrections?.length || 0;
  const accuracy = correctionCount > 0 
    ? 1 - (correctionCount / inferenceCount)
    : 0.87; // Default to training accuracy
