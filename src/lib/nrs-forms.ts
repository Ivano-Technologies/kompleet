...
export function generatePITForm(data: PITFormData): jsPDF {
  ...
  doc.setFontSize(14);
  doc.text('PERSONAL INCOME TAX RETURN', pageWidth / 2, 28, { align: 'center' });
  ...
  doc.text(`Date: ${new Date().toLocaleDateString('en-NG')}`, pageWidth - 50, 45);
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Consolidated Relief Allowance', `(${formatCurrency(data.consolidatedRelief)})`],
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Withholding Tax Credits', `(${formatCurrency(data.withholdingTax)})`],
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  const declaration = 'I declare that the information provided in this return is true, correct, and complete to the best of my knowledge.';
...
export function generateCITForm(data: CITFormData): jsPDF {
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Operating Expenses', `(${formatCurrency(data.operatingExpenses)})`],
  ...
  doc.autoTable({
    ...
    body: [
      ['Profit Before Tax', formatCurrency(data.profitBeforeTax)],
      ['Less: Capital Allowances', `(${formatCurrency(data.capitalAllowances)})`],
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  const declaration = 'I declare that this return has been examined by me and to the best of my knowledge and belief is correct and complete.';
  doc.text(declaration, 14, declStartY + 7, { maxWidth: pageWidth - 28 });
...
export function generateVATForm(data: VATFormData): jsPDF {
  ...
  doc.setFontSize(14);
  doc.text('VALUE ADDED TAX (VAT) RETURN', pageWidth / 2, 28, { align: 'center' });
  
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Period: ${data.period} | Tax Year: ${data.taxYear}`, pageWidth / 2, 35, { align: 'center' });
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Input VAT (VAT on Purchases @ 7.5%)', `(${formatCurrency(data.inputVAT)})`],
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.text('VAT returns must be filed within 21 days after the end of the relevant month/quarter.', 14, noteStartY + 6);
  doc.text('Late filing attracts penalties as prescribed by the Nigeria Tax Act 2025.', 14, noteStartY + 11);
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  const declaration = 'I declare that the particulars given in this return are true and correct.';
...
function addFooter(doc: jsPDF): void {
  ...
  doc.text('Generated by KOMPLEET Platform', pageWidth / 2, pageHeight - 15, { align: 'center' });
  doc.text('For official use only. Submit to Nigerian Revenue Service.', pageWidth / 2, pageHeight - 10, { align: 'center' });
  doc.text(`Generated on: ${new Date().toLocaleString('en-NG')}`, pageWidth / 2, pageHeight - 5, { align: 'center' });
}
