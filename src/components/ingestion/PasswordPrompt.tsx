...
export default function PasswordPrompt({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLocalError(
        err instanceof Error ? err.message : "Failed to unlock file",
      );
    } finally {
      ...
      <DialogContent className="sm:max-w-md backdrop-blur-md bg-white/95 dark:bg-slate-900/95 border border-white/20 dark:border-slate-800/20">
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Enter the password used to open this statement. We use it only to
            unlock the file and never store it.
          </p>
