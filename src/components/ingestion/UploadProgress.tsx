...
export default function UploadProgress({ progress, onCancel }: UploadProgressProps) {
  ...
  const getStepIcon = (step: ProgressStep) => {
    ...
    if (step === currentStep) {
      return <Loader className="w-5 h-5 text-green-600 dark:text-green-400 animate-spin" />;
    }
      ...
      <Card className="backdrop-blur-md bg-white/80 dark:bg-slate-900/80 border border-white/20 dark:border-slate-800/20">
        <div className="p-8 sm:p-12">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">{Math.round(progress)}%</p>
          </div>
