...
describe('Expense Sprint 1 – Core Data Model & Offline Engine', () => {
  ...
  describe('Supabase migration', () => {
    it('migration file exists and defines expense tables and RLS', () => {
      ...
      expect(sql).toMatch(/expenses_select|expenses_insert|expenses_update|expenses_delete/);
    ...
    it('migration seeds Nigerian default categories', () => {
      ...
      const sql = fs.readFileSync(migrationPath, 'utf-8');
      expect(sql).toMatch(/Transport \(Okada\/Fuel\)|Airtime\/Data|Market\/Inventory|VAT|Utilities|Logistics|Office Supplies/);
    });
  });
