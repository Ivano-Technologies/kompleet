...
import { hasPermission, hasAnyPermission, hasAllPermissions, getPermissions, ROLE_PERMISSIONS } from '../src/lib/auth/rbac';
...
describe('Role-Based Access Control (RBAC)', () => {
  ...
  describe('hasAnyPermission', () => {
    it('should return true if user has at least one permission', () => {
      expect(hasAnyPermission('user', ['admin:access', 'calculators:read'])).toBe(true); // has calculators:read
      expect(hasAnyPermission('admin', ['admin:manage_users', 'admin:access'])).toBe(true); // has admin:access
    });

    it('should return false if user has none of the permissions', () => {
      expect(hasAnyPermission('viewer', ['transactions:write', 'admin:access'])).toBe(false);
      expect(hasAnyPermission('user', ['admin:access', 'admin:manage_users'])).toBe(false);
    });
  });

  describe('hasAllPermissions', () => {
    it('should return true if user has all permissions', () => {
      expect(hasAllPermissions('owner', ['admin:access', 'admin:manage_users', 'export:bulk'])).toBe(true);
      expect(hasAllPermissions('user', ['calculators:read', 'transactions:read'])).toBe(true);
    });

    it('should return false if user is missing any permission', () => {
      expect(hasAllPermissions('admin', ['admin:access', 'admin:manage_users'])).toBe(false); // missing manage_users
      expect(hasAllPermissions('viewer', ['calculators:read', 'calculators:write'])).toBe(false); // missing write
    });
  });
