-- KOMPLEET Platform - Complete Database Migration (WORKING VERSION)
-- Version: 1.0.3
-- Description: Creates all core tables + RLS policies (fully qualified names per Supabase AI)
...
-- ============================================================================
-- PART 4: ROW LEVEL SECURITY (RLS) - FULLY QUALIFIED NAMES
-- ============================================================================
...
CREATE POLICY users_select_own ON public.users 
  FOR SELECT 
  USING (public.users.clerk_user_id = (current_setting('request.jwt.claims', true)::json->>'sub'));
...
CREATE POLICY users_insert_own ON public.users 
  FOR INSERT 
  WITH CHECK (public.users.clerk_user_id = (current_setting('request.jwt.claims', true)::json->>'sub'));

-- Categories: Can access own categories and system categories
CREATE POLICY categories_select ON public.categories 
  FOR SELECT 
  USING (
    public.categories.is_system = TRUE 
    OR public.categories.user_id IS NULL 
    OR public.categories.user_id = public.get_current_user_id()
  );

CREATE POLICY categories_insert_own ON public.categories 
  FOR INSERT 
  WITH CHECK (public.categories.user_id = public.get_current_user_id());

CREATE POLICY categories_update_own ON public.categories 
  FOR UPDATE 
  USING (
    public.categories.user_id = public.get_current_user_id() 
    AND public.categories.is_system = FALSE
  );
...
CREATE POLICY bank_accounts_all ON public.bank_accounts 
  FOR ALL 
  USING (public.bank_accounts.user_id = public.get_current_user_id());

-- Transactions: Full CRUD via bank account ownership (SUPABASE AI VERIFIED FIX)
...
-- ✅ VERIFIED: Fully qualified table names in all policies (Supabase AI confirmed)
