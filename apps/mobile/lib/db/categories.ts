...
function seedDefaultsIfNeeded(): void {
  const db = getDb();
  const rows = db.getAllSync<{ id: string }>('select id from expense_categories limit 1');
...
export function getCategoryNameById(id: string | null): string {
  if (!id) return '';
  const db = getDb();
  const rows = db.getAllSync<{ name: string }>('select name from expense_categories where id = ?', [id]);
  return rows[0]?.name ?? '';
}
