...
function generatePITForm(data) {
  ...
  doc.setFontSize(16);
  doc.text('Personal Income Tax (PIT) Return', pageWidth / 2, 25, { align: 'center' });
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Consolidated Relief', `(${data.consolidatedRelief.toLocaleString('en-NG')})`],
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Withholding Tax', '', `(${data.withholdingTax.toLocaleString('en-NG')})`],
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  const declaration = 'I declare that the information provided in this return is true, correct, and complete to the best of my knowledge and belief.';
  ...
  doc.text(`Generated by KOMPLEET Platform | ${new Date().toLocaleString('en-NG')}`, pageWidth / 2, 285, { align: 'center' });
  doc.text('For official use only | Nigeria Revenue Service', pageWidth / 2, 290, { align: 'center' });
...
function generateCITForm(data) {
  ...
  doc.setFontSize(16);
  doc.text('Company Income Tax (CIT) Return', pageWidth / 2, 25, { align: 'center' });
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Operating Expenses', `(${data.operatingExpenses.toLocaleString('en-NG')})`],
      ['Profit Before Tax', data.profitBeforeTax.toLocaleString('en-NG')],
      ['Less: Capital Allowances', `(${data.capitalAllowances.toLocaleString('en-NG')})`],
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Less: Advance Payments', '', `(${data.advancePayments.toLocaleString('en-NG')})`],
      ['Balance Due', '', data.balanceDue.toLocaleString('en-NG')]
    ],
    theme: 'striped',
    headStyles: { fillColor: [10, 104, 71], textColor: 255 },
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  const declaration = 'I declare that the information provided in this return is true, correct, and complete to the best of my knowledge and belief, and that this return has been prepared in accordance with the Companies Income Tax Act.';
  doc.text(declaration, 14, yPos, { maxWidth: 180 });
...
function generateVATForm(data) {
  ...
  doc.setFontSize(16);
  doc.text('Value Added Tax (VAT) Return', pageWidth / 2, 25, { align: 'center' });
  ...
  doc.autoTable({
    ...
    head: [['Description', 'Amount (₦)']],
    body: [
      ['Output VAT (7.5% on taxable supplies)', data.outputVAT.toLocaleString('en-NG')],
      ['Less: Input VAT (7.5% on purchases)', `(${data.inputVAT.toLocaleString('en-NG')})`],
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('Payment must be made within 21 days after the end of the quarter.', 14, yPos);
  yPos += 6;
