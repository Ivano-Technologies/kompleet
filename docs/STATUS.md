...
- Keep-alive is required infrastructure, not optional. `.github/workflows/keepalive.yml` (cron every 3 days) pings PostgREST directly (`tax_rules?select=id&limit=1` with `secrets.SUPABASE_PUBLISHABLE_KEY`) so the free-tier project does not pause. `GET /api/health/db` stays the uptime-monitor probe (Vercel `KEEPALIVE_TOKEN`); the workflow no longer uses it — GitHub Actions IPs are challenged by Cloudflare on the custom domain. Accepted, time-limited: GitHub disables scheduled workflows in public repos after 60 days without commits; this is a workaround, not a fix — Supabase Pro (~$25/mo) does not pause. Revisit before real users exist. Anon SELECT on `tax_rules` (migration `20260813210027`) exists for this ping; RLS still hides all rows from anon. Authenticated SELECT on `tax_rules` and `rule_versions` (migration `20260814042344`) is required for `GET /api/tax-rules` — hardening had revoked it while leaving the RLS policies in place.
- E2E is advisory (`continue-on-error` on the `e2e` job). `rls-negative` and the unit suite are the gates until a stable seeded test environment exists. Specs stay; they cover route protection on five paths.
