...
**Verified against:** `staging` tip after Phase 5 (IVA-64) mobile + `@supabase` dep + `supabase/` tree strip.  
**Phase 5:** Mobile sync/receipts use Path B `/api/expenses*` + Convex Auth Bearer. Root `@supabase/*` packages and the `supabase/` tree are gone. `health/db` keep-alive uses PostgREST `fetch` (no supabase-js). Schema-missing leftover APIs auth via Convex and return empty/accept-only responses.
...
**Scale (this tip):** Phase 5 strip complete on this branch. No `@supabase/*` in package.json. No `supabase/` tree. Mobile uses Convex Auth + `/api/expenses` + `/api/expenses/receipt`. Keep-alive still pings hosted PostgREST until Phase 6.
...
| **mobile** | **Moved (Phase 5).** Sync + receipts use Path B `/api/expenses*` + Convex Auth Bearer (`apps/mobile/lib/auth/convex-auth.ts`, `lib/api/client.ts`). Settings email/password sign-in. NDPR consent is local SecureStore only. | Existing `convex/expenses.ts` + `convex/files.ts` via Next APIs | Cutover done | **High** (field devices — smoke on Preview) | **2** then **10** |
...
| **documents / workers (3 APIs)** | **Moved (Phase 4).** SB adapters deleted in Phase 5. | Existing `convex/documents.ts` + `convex/audit.ts` + `convex/files.ts` | Cutover done | **High** | **5** |
| **AI / ML / feedback** | **Phase 5 strip.** Routes auth via Convex. `ai/categorize` + `ai/batch-categorize` use Convex categories (no `ml_inference_logs`). `ml/recurring` detects in-memory from Convex transactions. `feedback` / `ml/corrections` / `categorize` accept-only or empty (no schema). | Do not invent `convex/categorization.ts` | Auth cutover; persist deferred | Medium | **6** |
| **secondary APIs** | **Phase 5 strip.** Schema-missing routes (`deadlines/upcoming`, `reminders/history`, `notifications/preferences`, `migration/migrate`) use Convex auth + empty/accept/501. `health/db` keep-alive is PostgREST `fetch` (no supabase-js) until Phase 6. | Existing Convex modules; no new tables | Cutover except keep-alive ping | Medium | **7** (keep-alive last) |
...
| **lib shim** | **Deleted (Phase 5).** `src/lib/supabase/*` gone. Dead SB-only libs deleted (`reminder-job`, `data-migration-service`, `expense-premium`, `feedbackService`, `ml/monitoring`, `rules-engine`). Survivors stubbed or Convex-only. | — | Deleted | Medium | **9** |
| **deps + `supabase/` tree** | **Deleted (Phase 5).** No `@supabase/*` in package.json. `supabase/` tree gone. CI: `check:migrations` + `test:rls` are no-ops. `check:security-advisors` kept (no baseline bump). Backfill scripts use PostgREST/Storage REST. | — | Strip done | Medium (CI + advisor baseline) | **10** |
...
| `documents` | repository + workers | `documents` | `convex/documents.ts` | API + workers moved (Phase 4). Payload holds `documentType` / `fileUrl` / OCR fields. |
| `audit_logs` | leftover APIs + workers | `auditLogs` | `convex/audit.ts` | `audit-log` / `audit/log` / `history/*` moved. Document workers use `appendInternal`. |
...
| Documents pipeline | 5 | **Done (Phase 4).** Workers + `v1/documents` + `ingest` flipped together onto existing `documents` / `auditLogs`. |
...
| Leftover APIs in §2 | Phase 3–4 in-scope routes now use `requireAuthedConvex`. Still SB auth: AI/ML/feedback, deadlines/reminders, notifications, migration, `health/db`. |
...
| --- | --- |
| **1** | **This docs PR.** |
| **2+** | Execute kill-order 1–10 below. Original buckets: Phase 2 money leftovers; Phase 3 remaining APIs + libs; **Phase 4 documents/workers (done)**; Phase 5 mobile + deps + `supabase/` tree; Phase 6 soak → prod Convex URL → teardown. |
...
5. Documents pipeline (v1 + ingest + adapters + workers) — **done (Phase 4 / IVA-66)**
6. AI / ML / feedback
...
Also: `convex/backfill.ts` (internal backfill actions, including `internal.documents.upsertFromBackfill`). Checksums: [convex-backfill.md](./convex-backfill.md). Re-run before Phase 6 teardown.

---

## 8. Phase 4 completion notes (IVA-66)

Cut over on `staging` (this tip):

| Path | After |
| --- | --- |
| `POST /api/v1/documents/upload` | `requireAuthedConvex` + `ConvexDocumentRepository` + BullMQ |
| `GET /api/v1/documents/[id]/status` | `requireAuthedConvex` + `api.documents.getMine` |
| `POST /api/ingest` | Convex auth only (parse, no persist). Live persist stays `upload-v2` |
| `document-processor.worker` / `document-recovery.worker` | `createConvexAdminClient()` + `internal.documents.*` + `internal.audit.appendInternal` |
| Next 16 | `dynamic = "force-dynamic"`, `rethrowIfNextControlFlow`, no Response clone in `withRateLimit`, `cookies()` before catch-all |

Worker env (in addition to `REDIS_URL`): `NEXT_PUBLIC_CONVEX_URL` or `CONVEX_URL`, plus `DOCUMENT_WORKER_TOKEN` (min 16 chars) on **both** the worker process and the Convex deployment (`npx convex env set DOCUMENT_WORKER_TOKEN ...` / dashboard). Convex HTTP clients in this repo cannot call `internal.*`, so workers use public `worker*` / `appendForWorker` functions gated by that token. Do not use `SUPABASE_SERVICE_ROLE_KEY` for these workers after this cut.

`health/db` still pings hosted PostgREST `tax_rules` (no supabase-js). Prod `NEXT_PUBLIC_CONVEX_URL` stays off. Phase 6 owns soak → prod Convex URL → project teardown.
